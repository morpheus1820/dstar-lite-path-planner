...
Dstar::Dstar() { 
  ...
  maxSteps = 160000; //80000;  // node expansions before we give up
...
void Dstar::drawWaypoints()
{
  list<waypoint>::iterator iter2;


  glColor3f(0.4, 0.4,0.8);
  for(iter2=waypoints.begin(); iter2 != waypoints.end(); iter2++) 
  {
  	glBegin(GL_LINE_LOOP);
	for (int i=0; i<360; i++)
	{
	  float degInRad = i*(3.14159/180);
	  glVertex3f(cos(degInRad)*iter2->radius + iter2->x,sin(degInRad)*iter2->radius+ iter2->y,0.2);
	} 
	  glEnd();
  }
}
...
double Dstar::haversine_km(double lat1, double long1, double lat2, double long2)
{
